    ...
    public function upload(Request $request)
    {
        ...
        if ($file) {
            ...
            if (!in_array($ext_suffix, $allow_suffix)) {
                return $this->response->array(['code' => 1001, 'message' => '上传的文件类型只能是jpg,gif,jpeg,png']);
            }

            $material = new AliOssService();
            $result = $material->upload($file);
            if ($result) {
                return $this->response->array(['code' => 0, 'file' => $result, 'width' => $width, 'height' => $height
                    , 'type' => $type, 'message' => 'success']);
            } else {
